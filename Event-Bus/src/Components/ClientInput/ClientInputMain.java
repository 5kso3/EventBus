...
public class ClientInputMain {
		...
		while (!done) {
			...
			try {
				...
				case "1":
					...
					break;
				case "2":
					eventBus.sendEvent(new Event(EventId.DeleteStudents, makeDeleteStudentInfo()));
				...
				case "5":
					...
					break;
				case "6":
					eventBus.sendEvent(new Event(EventId.DeleteCourses, makeDeleteCourseInfo()));
					printLogSend(EventId.RegisterCourses);
					break;
				case "7":
					eventBus.sendEvent(new Event(EventId.Enrollment, makeEnrollmentInfo()));
					printLogSend(EventId.Enrollment);
					break;
				case "0":
	...
	private static String makeEnrollmentInfo(){
	...
	private static String makeDeleteStudentInfo() throws IOException {
		String userInput = "";
		System.out.println("\nEnter student ID and press return (Ex. 20131234)>> ");
		userInput = new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
		return userInput;
	...
	private static String makeDeleteCourseInfo() throws IOException {
		String userInput = "";
		System.out.println("\nEnter course ID and press return (Ex. 12345)>> ");
		userInput = new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
		return userInput;
	...
	private static void writeMenu() {
		...
		System.out.println("6. Register a new Course");
